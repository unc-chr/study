...
void* thrRun(void* arg) {
    ...
    while (1) {
        // 获取任务，先要尝试加锁
        pthrad_mutex_lock(&thrPool->pool_lock);
        ...
        if (thrPool->shutdown) {
            // 要销毁线程池，此时线程退出即可
            // pthread_detach(pthrea_self()); // 临死前分家
...
void create_threadpool(int thrnum, int maxtasknum) {
    ...
    thrPool->shutdown = 0; // 是否销毁线程池，1 代表销毁
...
void addtask(ThreadPool* pool) {
    ...
    pthread_cond_signal(&pool->not_empty_task);  // 通知包身工，what the fuck?
    printf("end call %s-----\n", __FUNCTION__);
}
